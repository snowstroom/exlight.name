...
export class CommentService extends Api {
    ...
    public async addComment(articleId: number, comment: ArticleNamespace.IArticleCommentary): Promise<any> {
        try {
    ...
    public async updateComment(articleId: number, comment: ArticleNamespace.IArticleCommentary): Promise<any> {
        try {
    ...
    public async getCommentList(articleId: number, pagParams: PaginationParams): Promise<Commentary[]> {
        try {
            const params = pagParams.getUrlString();
            const comments = await this.get<ArticleNamespace.IArticleCommentary[]>(`commentary/list/article/${articleId}${params}`);
